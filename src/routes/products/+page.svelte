        ...
        <div class="grid">
          {#each data.products as product (product.id)}
            ...
            >
              <div class="img-wrap">
                <Image
                  src={product.models?.flatMap(
                    (m) => m.shootings?.map((s) => s.image) ?? [],
                  ) ?? []}
                  interval={5000}
                />
              </div>
              <div class="info">
                <div class="branding">
                  <h3 class:smaller={product.name.length > 16}>
                    {product.name}
                  </h3>
                  <p class="meta">
                    {shapes[product.shape]} | {categories[product.category]}
                  </p>
                </div>
